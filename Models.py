...
class CombinedModel(nn.Module):
    ...
    def kmedoids_update(self, max_k=10):
        """
        K-Medoids 量化模型权重
        :param max_k: 表示K-Medoids 聚类中的最大簇数，默认值为10
        """
        # FedAnil+: Silhouette Index
        # 初始化变量 存储最高的轮廓系数分数 存储最佳的簇数 存储最佳的聚类结果
        max_silhouette_scores = 0
        best_k = 2
        best_kmedoids_data = {}
        for k in range(2, max_k + 1):
            # 循环尝试不同的k值，找到最佳的簇数
            kmedoids_clusters_and_labels = dict()  # 初始化一个空字典，用于存储当前 k 值下的聚类结果
            # FedAnil+: Silhouette Index 累积所有变量的轮廓系数分数和计数
            sum_silhouette_scores = 0
            vars_count = 0
            for var in self.state_dict():
                shape_of_datas = self.state_dict()[var].shape  # [64,100]
                datas = self.state_dict()[var].reshape(shape_of_datas[0], -1).cpu()
                k = min(k, shape_of_datas[0] - 1)
                datakm = KMedoids(n_clusters=k, random_state=0).fit(datas)  # 执行K-Medoids聚类算法,random_state=0确保结果的可重复性
                kmedoids_clusters_and_labels[var] = datakm  # 将聚类结果保存到字典中
                with open('./test/kmedoids_clusters_and_labels_k_2.txt', 'w') as f:
                    for key, value in kmedoids_clusters_and_labels.items():
                        f.write('{}\t:{}\n'.format(key, value))
                # 如果聚类结果包含多个簇，计算并累积轮廓系数分数，并增加变量计数。
                # datakm.labels_ 包含了每个数据点所属的簇标签，轮廓系数的值范围从 -1 到 1，值越高表示聚类效果越好，即同一簇内的点更相似，不同簇之间的点更不相似
                if (np.unique(datakm.labels_).size > 1):
                    sum_silhouette_scores += silhouette_score(datas, datakm.labels_)
                vars_count += 1
            avg_silhouette_score = sum_silhouette_scores / vars_count  # 计算所有变量的平均轮廓系数分数
            # 如果当前的平均轮廓系数分数高于之前记录的最高分数，则更新最佳聚类结果
            if avg_silhouette_score > max_silhouette_scores:
                max_silhouette_scores = avg_silhouette_score
                best_k = k
                best_kmedoids_data = kmedoids_clusters_and_labels
        print('The best_kmedoids_data is : {}'.format(best_kmedoids_data))
        print('The best_k is : {}'.format(best_k))
        return best_kmedoids_data
